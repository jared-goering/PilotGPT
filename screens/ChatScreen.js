...
import { View, Text, TextInput, ScrollView, Image, TouchableOpacity, ActivityIndicator, KeyboardAvoidingView, Platform, ImageBackground } from 'react-native';
...
export default function ChatScreen({ navigation }) {
    ...
    const handleQuery = async () => {
        ...
        setConversation((prevConversation) => [...prevConversation, userMessage, thinkingMessage]);
        setQuery('');
        setLoading(true);

        try {
            const response = await axios.post('http://localhost:3001/query', { query, userId, threadId });
            console.log("threadId from response", response.data.threadId);
            const newMessage = {
                role: 'assistant',
    ...
    return (
        <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
            <View style={styles.header}>
                <ImageBackground source={require('../assets/wave.png')} style={styles.headerWave} />
            ...
            <View style={styles.chatContainer}>
                <ScrollView style={styles.conversationContainer} ref={scrollViewRef} contentContainerStyle={{ paddingBottom: 20 }}>
